...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "view") {
      if (!SLUG_RE.test(slugRaw) || slugRaw === "default") {
        return jsonResponse({ error: "无效的名片 ID" }, 400);
      }
      const deviceId = String(body.deviceId || "").trim().slice(0, 64);
      if (!deviceId) {
        return jsonResponse({ error: "缺少设备标识" }, 400);
      }
      ...
      if (!cardRow) {
        return jsonResponse({ ok: true, recorded: false, viewCount: 0 });
      }

      const { data: seen } = await supabase
        .from("card_view_devices")
        .select("slug")
        .eq("slug", slugRaw)
      ...
      if (seen) {
        return jsonResponse({ ok: true, recorded: false, viewCount: cardRow.view_count || 0 });
      }

      await supabase.from("card_view_devices").insert({ slug: slugRaw, device_id: deviceId });
      const newCount = (cardRow.view_count || 0) + 1;
      await supabase.from("user_cards").update({ view_count: newCount }).eq("slug", slugRaw);

      return jsonResponse({ ok: true, recorded: true, viewCount: newCount });
    }
  ...
  // save
    ...
    if (existing) {
      ...
      if (await slugExists(supabase, slugRaw)) {
        return jsonResponse({ error: "名片 ID 冲突，请刷新后重试" }, 409);
      }
