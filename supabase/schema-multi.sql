...
--
-- 若你已有 user_cards 表，只需加访问统计，请执行：
--   supabase/schema-migrate-view-count.sql
...
-- 已有表时补字段（view_count 必须先于下方 cards_public 视图）
...
alter table public.user_cards add column if not exists view_count integer not null default 0;
...
-- ========== 第三步：重建公开视图（依赖 user_cards.view_count 已存在） ==========

-- 已有 cards_public 时必须先 drop，不能用 create or replace 在中间插入列
